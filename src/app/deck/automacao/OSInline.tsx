...
const DEFAULT_SERVICES: ServiceItem[] = [
  { id: 1, description: 'Diagnóstico e mapeamento de processos', qty: '1', price: '' },
  { id: 2, description: 'Desenvolvimento da automação', qty: '1', price: '' },
  { id: 3, description: 'Testes, entrega e treinamento', qty: '1', price: '' },
];
...
export default function OSInline() {
  ...
  const generatePdfBase64 = async (): Promise<string | null> => {
    try {
      ...
      // Proposta block (right)
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(7);
      doc.setTextColor(...orange);
      doc.text('PROPOSTA COMERCIAL — AUTOMAÇÃO', W - m, y - 5, { align: 'right' });
      doc.setFontSize(18);
      doc.setTextColor(...black);
      ...
      doc.text(`Gerado por Salto · saltoup.com · Proposta #${osNum} · ${date}`, W / 2, 285, { align: 'center' });
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/os-automacao', {
        method: 'POST',
  ...
  const buildText = () => [
    `*PROPOSTA COMERCIAL — Automação · Salto*`,
    `Proposta #${osNum} · ${date}`,
    ``,
      ...
      <div style={{ width: '100%', maxWidth: 680, background: '#fff', borderRadius: 14, padding: '28px 36px', color: '#1a1a1a', boxShadow: '0 16px 60px rgba(0,0,0,0.6)', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, paddingBottom: 16, borderBottom: '2px solid #111' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <p style={{ fontSize: 9, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', color: '#999', marginBottom: 3 }}>Proposta Comercial · Automação</p>
