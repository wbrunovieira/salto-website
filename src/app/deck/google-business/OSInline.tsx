...
export default function OSInline() {
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      if (res.ok) { setEmailStatus('sent'); setShowFicha(true); }
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', paddingBottom: 16 }}>
        ...
        {emailStatus === 'sent' && (
          ...
          >
            ...
            {showFicha ? 'Ocultar Ficha' : 'Ficha Técnica'}
          </button>
        )}
      </div>

      {/* Ficha Técnica — abre/fecha via botão, estado preservado enquanto a aba estiver aberta */}
      {showFicha && (
        <div style={{ width: '100%', maxWidth: 680 }}>
          <div style={{ background: '#fff', borderRadius: 14, padding: '28px 36px', color: '#1a1a1a', boxShadow: '0 16px 60px rgba(0,0,0,0.6)' }}>
            <FichaTecnica
              osNum={osNum}
              date={date}
