...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!process.env.RESEND_API_KEY) {
      console.warn('RESEND_API_KEY not set — OS website email not sent');
    ...
    await resend.emails.send({
      ...
      to: TO_EMAIL,
      subject: `🌐 Nova OS Website #${osNum} — ${empresa}`,
    ...
    if (email) {
      const { error: clientErr } = await resend.emails.send({
        from: `Bruno Vieira | WB Digital <${FROM_EMAIL}>`,
        to: email,
        replyTo: TO_EMAIL,
        subject: `Sua Ordem de Serviço #${osNum} — WB Digital Solutions`,
        react: OrdemServicoCliente({ osNum, date, nomeFantasia, responsavel, telefone, items, obs, total }),
        attachments: attachment,
        tags: [
          { name: 'tipo', value: 'os-website' },
      ...
      if (clientErr) {
        console.error('OS website client email error:', clientErr);
        clientEmailError = email;
        await resend.emails.send({
          from: `Salto Alerta <${FROM_EMAIL}>`,
          to: TO_EMAIL,
          subject: `⚠️ E-mail inválido na OS Website #${osNum} — ${empresa}`,
          html: `
    ...
    console.error('OS website email error:', err);
